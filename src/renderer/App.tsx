...
export function App() {
    ...
    const handleSave = (entry: PasswordEntry) => {
        setPasswords((prev) => {
            ...
            return exists ? prev.map((p) => (p.id === entry.id ? entry : p)) : [...prev, entry];
        });
        setShowFormModal(false);
        setEditingEntry(null);
    };
    ...
    const handleMoveEntry = (entryId: string, targetPath: string) => {
        setPasswords((prev) => prev.map((p) => (p.id === entryId ? { ...p, group: targetPath } : p)));
    };
    ...
    const handleRenameFolder = (path: string, newLeaf: string) => {
        ...
        if (!trimmed) return;
        const parent = path.includes(PATH_SEP) ? path.slice(0, path.lastIndexOf(PATH_SEP)) : "";
    ...
    // フォルダを削除（配下のエントリもサブフォルダごと削除）。確認は FolderList 側のモーダルで実施
    const handleDeleteFolder = (path: string) => {
        ...
        setSelectedFolder((sel) => (sel === path || (sel && sel.startsWith(path + PATH_SEP)) ? ALL_FOLDERS : sel));
    };
    ...
    const folderEntries = passwords.filter((entry) => {
        ...
        return group === selectedFolder || group.startsWith(selectedFolder + PATH_SEP);
    });
    ...
    const folderLabel =
        selectedFolder === ALL_FOLDERS ? (
            ...
            selectedFolder.split("/").map((segment, index) => (
                <React.Fragment key={index}>
                    {index > 0 && <ChevronRight className="h-4 w-4 shrink-0 text-slate-500" />}
                    <span>{segment}</span>
                </React.Fragment>
            ))
        );
    ...
    return (
        <div className="flex h-screen flex-col overflow-hidden text-slate-200">
            ...
            {authState === "loading" ? (
                <div className="flex flex-1 items-center justify-center text-slate-500">読み込み中...</div>
            ) : authState !== "unlocked" ? (
                ...
                <div className="flex min-h-0 flex-1 overflow-hidden">
                    ...
                    <main className="flex-1 flex flex-col min-w-0">
                        {passwords.length === 0 ? (
                            <div className="flex-1 flex items-center justify-center p-8">
                                <div className="text-center animate-fade-in">
                                    ...
                                    <p className="text-slate-400 mb-8 max-w-sm mx-auto">
                                        新規登録するか、KeePass の CSV をインポートして、
                                        <br />
                                        あなたの認証情報を安全に管理しましょう。
                                    </p>
                                    <div className="flex items-center justify-center gap-3">
                                        <Button
                                            onClick={handleNew}
                                            size="lg"
                                            className="bg-gradient-to-r from-cyan-400 to-violet-500 text-slate-950 hover:shadow-[0_8px_30px_-6px_rgba(34,211,238,0.6)] hover:brightness-110"
                                        >
                            ...
                            <>
                                {/* ヘッダー：フォルダ名 + 検索 */}
                                <div className="flex items-center justify-between gap-4 px-8 py-2 border-b border-white/10 bg-white">
                                    ...
                                    <div className="flex items-center gap-3 shrink-0">
                                        <div className="relative w-64">
                                            <Search className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-500 z-10" />
                                            <Input
                                                type="text"
                                                placeholder="検索..."
                                                value={searchTerm}
            ...
            {/* インポートモーダル */}
            {showImportModal && (
                <ImportModal onSuccess={handleImportSuccess} onClose={() => setShowImportModal(false)} />
            )}
